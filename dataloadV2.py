...
def main():
    ...
    # Labels processing
    com_labels = ['yes', 'no', 'up', 'down', 'left', 'right', 'on', 'off', 'stop', 'go']
    label_dict = dict(zip(com_labels, range(len(com_labels))))


    for d in all_dir[:-1]:
        all_files = np.array(listdir(data_dir+"\\"+d))
        start_time = time.time()        # Used to evaluate time performances
        data_temp = pd.DataFrame(columns=['filename', 'label', 'label_num', 'data'])     # Initialize sub-dataframe
        for f in all_files:
            ...
            if src.shape[0] != 100:
                continue
            # Remove energy values and convert to list
            src = list(src[:, :-3].flatten())
            lab = label_dict.get(d, len(label_dict))
            # Update sub-dataframe
            temp = pd.DataFrame([[d+"/"+f, d, lab, src]], columns=['filename', 'label', 'label_num', 'data'])
    ...
    # Save all the sets
    #data_test.to_pickle('dTest.pkl')
    #data_val.to_pickle('dVal.pkl')
    data_test.to_hdf('dTestV2.h5', key='df', mode='w')
    data_val.to_hdf('dValV2.h5', key='df', mode='w')
